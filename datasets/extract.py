...
interrogation_bio = [
    ...
    "Nomenclature BiologiqueRessource en biologie",
    "Taxinomie",
]
...
def get_filtered_categorymembers(
    categorymembers, titles, to_ignore=[], level=0, max_level=1
):
    for k, c in categorymembers.items():
        if c.ns == wikipediaapi.Namespace.CATEGORY:
            if (level < max_level) and (k not in to_ignore):
                get_filtered_categorymembers(
                    categorymembers=c.categorymembers,
                    titles=titles,
                    level=level + 1,
                    max_level=max_level,
                )
...
def extract_wikipedia(
    category_names: list = ["Médecine", "Pharmacie", "Biologie"], lang: str = "fr"
) -> dict:
    wiki_wiki = wikipediaapi.Wikipedia(
        user_agent="Articles liées au domaine biomédical", language=lang
    )
